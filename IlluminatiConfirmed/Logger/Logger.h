...
#define INIT_LOG(mode, puth, className) IlluminatiConfirmed::Logger logger = ManagerLoggers::Instance(mode, puth).getLogger(className);
#define LOGGER(className) ManagerLoggers::Instance().getLogger(className)<< __PRETTY_FUNCTION__ << "\n"
...
namespace IlluminatiConfirmed
{
    ...
    /*!
     * \brief The TypeLog enum Unrealized feature. Has not thought out how to use and whether it is necessary at all.
     */
    enum TypeLog {
        DEBUG,
        INFO,
    ...
    public:
         ...
         * \param mode Determines the recording mode: either for each class its own file, or all is written in one log
         * \param puth Path to the storage location of the created log, must be a slash at the end.
         * \return
         */
        static ManagerLoggers& Instance(ModeSave mode = multiStream, /*TypeLog type = DEBUG,*/ std::string puth = "")
        {
         ...
         * \return
         */
        Logger& getLogger(const std::string &name = "multilog"/*, TypeLog level = DEBUG*/)
        {
    ...
    private:
         ...
         * \param puth
         */
        ManagerLoggers(ModeSave mode = multiStream, /*TypeLog type = DEBUG,*/ const std::string &puth = "") :
