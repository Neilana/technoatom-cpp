...
template <class Tp>
BugNumbers<Tp>::BugNumbers(const Tp &def)
    : Array<Tp, 1>(def), m_count({0, 0, 0}) {}

template <class Tp>
BugNumbers<Tp> &BugNumbers<Tp>::operator*(BugNumbers<Tp> &rhs) {
  if (this->m_count.multiplication > 3) {
    std::cout << "Pay me " << this->m_count.multiplication
              << " $ for operator*." << std::endl;
    return *this;
  } else {
    if (rhs.m_count.multiplication > 3) {
      std::cout << "Pay me " << 10 << " $ for operator* for right element."
                << std::endl;
      return rhs;
    }
  }
...
BugNumbers<Tp> BugNumbers<Tp>::operator==(BugNumbers<Tp> &rhs) {
  if (this->m_count.comparison > 3) {
    std::cout << "Pay me " << this->m_count.comparison * 10
              << " $ for operator==, otherwise false." << std::endl;
    return false;
  } else {
    if (rhs.m_count.comparison > 3) {
      std::cout << "Pay me " << this->m_count.comparison * 10
                << " $ for operator== for right element, otherwise false."
                << std::endl;
      return false;
    }
  }
