...
void Game::initPhysics()
{
    ...
    for (int i = 0; i < walls.size(); i++)
    {
        ...
        bodyDef.position.Set(walls[i].m_rect.left + tileSize.x / 2 * (walls[i].m_rect.width / tileSize.x - 1),
                             walls[i].m_rect.top + tileSize.y / 2 * (walls[i].m_rect.height / tileSize.y - 1));
    ...
    // загружаем в Box2D как основного игрока панду :) (потом надо будет всех, сейчас просто
...
{
    // здесь ничё не работает, надо химичить с координатами. в Box2D там всё в системе Си,
    // а нам нужны пиксели
    playerBody->SetLinearVelocity(b2Vec2(m_heroes[1].vx / 64.0, m_heroes[1].vy / 64.0 ));

    // выводим в консоль координаты Box2D и координаты панды (должны совпадать - но пока нет)
    ...
    world->Step(1.0f / 60.0f, 1, 1); // обновляем мир Box2D, не факт что оно здесь должно быть
}
