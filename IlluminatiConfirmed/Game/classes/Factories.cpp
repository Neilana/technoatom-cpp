...
FactoryObjects::create_character(const std::string &id, b2World *world) {
  ...
  QString str =
      "SELECT * FROM CharactersImages WHERE Id=" + QString::fromStdString(id);
  QSqlQuery query;
  query.exec(str);
  while (query.next()) {
    std::string fileName =
    ...
    auto pers = std::shared_ptr<experimental::BaseInterface>(
        characters_factory.get(master)(
            world, p_texture.get(),
            experimental::CharacterSpriteInfo(
                {width, height, DEFAULT_SPRITE_SIZE_X, frames, 300, 300})));
