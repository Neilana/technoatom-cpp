...
//}
///*
//std::shared_ptr<sf::SoundBuffer> FactoryObjects::getSoundBuffer(const std::string &file) {
//    auto p_buffer = std::make_shared<sf::SoundBuffer>();
//    auto it = m_vec_of_soundBuff.find(file);

//    if (it == m_vec_of_soundBuff.end()) {
//        if (!p_buffer->loadFromFile(file))
//            EXCEPTION(std::string("Resource hasn't opened, puth: ") + file, nullptr);

//        m_vec_of_soundBuff.insert({std::move(file), p_buffer});
//    } else
//        p_buffer = (*it).second;
//    return p_buffer;
//}
