...
/*!
 * \brief The Iterator class An Iterator is any object that, pointing to some
 * element in
 *              a range of elements (such as an array or a container), has the
 * ability to iterate
...
template <class Tp>
inline std::ptrdiff_t operator-(const Iterator<Tp>& lhs,
                                const Iterator<Tp>& rhs) {
...
class Iterator<bool> {
 public:
  ...
  // !!! для + - надо запилить проверки на выход за 0 и за макс размер
  // контейнера
  ...
  /*  inline bool operator<(const self_type& rhs) const
    ...
    self_type operator=(const self_type& rhs) { m_bitPtr.m_dataPtr =
    rhs.m_bitPtr.m_dataPtr; m_bitPtr.m_index = rhs.m_bitPtr.m_index; return
    *this; }
  */
  // inline pointer base() const { return m_ptr;}
