...
TEST(ArrayTest, CheckCopyConstructor) {
  ...
  // and
  int j = 1;
  for (auto &it : a2) ASSERT_EQ(it, j++ * 10);
}
...
TEST(ArrayTest, CheckSwap) {
  Array<double, 5> a1;
  for (size_t i = 0; i < 5; i++) a1[i] = i + 10;

  Array<double, 5> a2;
  for (size_t i = 0; i < 5; i++) a2[i] = i + 1000;
...
TEST(ArrayTest, CheckAggregateInitialization) {
  ...
  for (size_t i = 0; i < a1.size(); i++) ASSERT_EQ(a1.at(i), (i + 1) * 10);

  // test 2
  Array<double, 6> a2{10, 20, 30, 40, 50, 60};

  for (size_t i = 0; i < a2.size(); i++) ASSERT_EQ(a2.at(i), (i + 1) * 10);

  // test 3
  Array<double, 6> a3({10, 20, 30, 40, 50, 60});

  for (size_t i = 0; i < a3.size(); i++) ASSERT_EQ(a3.at(i), (i + 1) * 10);

  Array<double, 4> a5 = {10, 10.1, 10.2, 10.3};

  ASSERT_EQ(a5.size(), 4);
  double i = 0;
  for (auto &it : a5) ASSERT_EQ(it, 10.0 + i++ * 0.1);
