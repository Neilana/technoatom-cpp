...
// bool
// Vector<bool>::Vector() : ContainerInterface<bool>(nullptr, 0),
...
Vector<bool>::Vector(size_t capacity)
    : ContainerInterface<bool>(nullptr, 0), m_data(nullptr), m_capacity(0) {
  DUMP("in");
  try {
...
Vector<bool>::Vector(size_t capacity, const bool& value)
  ...
  try {
    ...
    for (size_t i = 0; i <= capacity / 8; i++) m_data[i] = intValue;
...
void Vector<bool>::assign(size_t capacity, const bool& value) {
  ...
  this->m_size = capacity;
  for (auto it = this->begin(); it != this->end() + capacity - this->m_size;
       it++)
    *it =
        value;  //на сотой итерации изменения кода прошел тест. Я у мамы упорный
}
