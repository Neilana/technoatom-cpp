...
TEST(VectorTest, CheckSwap) {
  ...
  for (size_t i = 0; i < 10; i++) v1[i] = i + 10;

  Vector<double> v2(5, 0.0);
  for (size_t i = 0; i < 5; i++) v2[i] = i + 1000;
...
TEST(VectorTest, CheckResize) {
  ...
  v1.resize(5);
  for (size_t i = 0; i < 5; i++) ASSERT_EQ(v1[i], i * 10);
  ...
  v2.resize(10);
  for (size_t i = 0; i < 5; i++) ASSERT_EQ(v2[i], i * 1000);

  for (size_t i = 5; i < 10; i++) ASSERT_EQ(v2[i], 0);
}
...
TEST(VectorTest, CheckAggregateInitialization) {
  ...
  for (size_t i = 0; i < v1.capacity(); i++) ASSERT_EQ(v1.at(i), (i + 1) * 10);

  // test 2
  Vector<double> v2{10, 20, 30, 40, 50, 60};

  for (size_t i = 0; i < v2.capacity(); i++) ASSERT_EQ(v2.at(i), (i + 1) * 10);

  // test 3
  Vector<double> v3({10, 20, 30, 40, 50, 60});

  for (size_t i = 0; i < v3.capacity(); i++) ASSERT_EQ(v3.at(i), (i + 1) * 10);
}
...
TEST(VectorTest, CheckNewElement) {
  ...
  Vector<int> *v2 = new (0) Vector<int>(
      3);  // i don't know who might use that form and why... but it works
...
TEST(VectorIteratorsTest, CheckIterators)  //эммм..))
{
  ...
  size_t i = 0;
  ASSERT_EQ(6, v1.size());
  for (IlluminatiConfirmed::Vector<double>::iterator it = v1.begin();
       it != v1.end(); it++) {
    ASSERT_EQ((*it), ((i++) + 1) * 10);
  }
  ...
  i = 0;
  for (IlluminatiConfirmed::Vector<double>::const_iterator it = v2.begin();
       it != v2.end(); it++) {
...
{
  // IlluminatiConfirmed::ContainerInterface<double> s; //It does not compile if
  // all is ok
}
...
TEST(VectorTest, CheckInsert) {
  ...
  int i = 1;
  for (auto it : s1) ASSERT_EQ(it, (i++) * 10);
  ...
  i = 1;
  for (auto it : s3) ASSERT_EQ(it, (i++) * 10);
  ...
  i = 1;
  for (auto it : s4) ASSERT_EQ(it, (i++) * 10);
}
