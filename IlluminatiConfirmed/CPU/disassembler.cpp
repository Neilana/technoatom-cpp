...
void Dissasembler::runDisassemblerForTxtFile(const std::string &input, const std::string &output)
{
...
void Dissasembler::runDisassemblerForMemory(const std::vector<CPU::value_type> &memory, const std::string &output)
{
    if (m_memory != memory)
        m_memory = memory;
    m_stringList.clear();
