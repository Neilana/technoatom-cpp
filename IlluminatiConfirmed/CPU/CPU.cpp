...
const std::map<Command, CommandInfo> CPU::info = {
    ...
    {Command::PushReg,
     {static_cast<CPU::value_type>(Command::PushReg), 1, "pushReg"}},
    {Command::PushConst,
     {static_cast<CPU::value_type>(Command::PushConst), 1, "pushConst"}},
    ...
    {Command::Label,
     {static_cast<CPU::value_type>(Command::Label), 0, "label"}}};
...
std::map<Command, CPU::CommandInfoCPU> CPU::makeInfo() {
  ...
  std::map<Command, CPU::CommandInfoCPU> info = {
      ...
      {Command::Add, {[this, lambdaMath](auto &&it) {
         lambdaMath(it, ([](auto &&buf1, auto &&buf2) -> auto {
                      return buf1 + buf2;
                    }));
       }}},
      ...
      {Command::Jmp, {[this](auto &&it) { it = m_memory.begin() + *++it; }}},
      {Command::Ja, {[this, lambdaJamp](auto &&it) {
         lambdaJamp(it, [](auto &&first, auto &&second) -> bool {
           return first > second;
         });
       }}},
      ...
      {Command::End, {[](auto &&itBuf) { ++itBuf; }}},
      {Command::Label, {[](auto &&itBuf) { (void)itBuf; }}}};
  return info;
}

void CPU::setStack(const std::stack<value_type> &stack) { m_stack = stack; }
