...
class Dissasembler {
 public:
  /*!
   * \brief Dissasembler Throw IlluminatiConfirmed::Exception if not all the
   * commands are defined
   */
  Dissasembler() : m_memory(CPU::MEMORY_CAPACITY), argCount(0) {
    if (CPU::info.size() != m_commandsInfoDisassembler.size())
      throw EXCEPTION("Not all the commands are defined in the disassembler",
                      nullptr);
  }

  /*!
   * \brief runDisassemblerForTxtFile Runs d isassembler for txt File, ,
   * IlluminatiConfirmed::Exception if cannot open file
   * \param input
   * \param output
   */
  void runDisassemblerForTxtFile(const std::string &input,
                                 const std::string &output);

  /*!
   * \brief runDisassemblerForMemory Runs disassembler for memory,
   * IlluminatiConfirmed::Exception if cannot open file
   * \param memory
   * \param output
   */
  void runDisassemblerForMemory(const std::vector<CPU::value_type> &memory,
                                const std::string &output);
 ...
 private:
  ...
  /*!
   * \brief saveStringListToFile Saves string list to file,
   * IlluminatiConfirmed::Exception if cannot open file
   * \param output
   */
  ...
  const std::map<Command, CommandInfoDissassembler> m_commandsInfoDisassembler =
      makeInfo();
};
}
