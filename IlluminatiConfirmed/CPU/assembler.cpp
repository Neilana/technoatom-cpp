...
std::map<Command, Assembler::CommandInfoAssembler> Assembler::makeInfo() {
  auto lambdaLabel = [this](auto &&itBuf) {
    ...
    m_memory.push_back((temp_iter != m_labels.end()) ? ((*temp_iter).second)
                                                     : (0));
  };

  std::map<Command, Assembler::CommandInfoAssembler> info = {
      {Command::Push, {[this](auto &&itBuf) {
         m_commandsInfoAssembler
             .at(((*(itBuf + 1)).at(0) == 'x') ? (Command::PushReg)
                                               : (Command::PushConst))
             .parse(itBuf);
       }}},
...
void Assembler::runAssemblerForFile(const std::string &input) {
  ...
  // int strCount = 0;
  std::string temp;
  while (fileInput >> temp) buffer.push_back(temp);
  fileInput.close();
...
void Assembler::oneAsmPass(std::vector<std::string> &buffer) {
  ...
  for (auto itBuf = buffer.begin(); itBuf != buffer.end(); ++itBuf) {
    Command cmd =
        ((*itBuf).back() == ':') ? (Command::Label) : (tokens.at(*itBuf));
    // throw EXCEPTION((std::string("Unknown cmd: ") + (*itBuf)), nullptr);
    // if we have enough memory
    if ((m_memory.size() + 1 + CPU::info.at(cmd).argsCount) <
        m_memory.capacity()) {
      // if (cmd != Command::Label) //The only command that doesn't need to be
      // typed
      //    m_memory.push_back((CPU::info.at(cmd).id));
      m_commandsInfoAssembler.at(cmd).parse(itBuf);
    } else
      EXCEPTION("Memory is over", nullptr);
  }
}
...
void Assembler::saveMemoryToTextFile(const string &fileName) {
  ...
  if (!saveFile.is_open())
    throw EXCEPTION("File for saving has not opened", nullptr);
  for (auto &it : m_memory) saveFile << it << " ";
  saveFile.close();
}
