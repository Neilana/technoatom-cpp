...
TEST(cpu, run) {
  ...
  std::vector<CPU::value_type> memory = {CPU::info.at(Command::PushConst).id,
                                         100,
                                         CPU::info.at(Command::PushConst).id,
                                         200,
                                         CPU::info.at(Command::Add).id,
                                         CPU::info.at(Command::Pop).id,
                                         1,
                                         CPU::info.at(Command::End).id};
  CPU cpu;
  cpu.setMemory(memory);
  std::array<CPU::value_type, CPU::REGISTERS_COUNT> registers = {0, 0, 0,
                                                                 0, 0, 0};
  cpu.setRegisters(registers);
  cpu.run();

  // when
  std::array<CPU::value_type, CPU::REGISTERS_COUNT> excpected = {
      0, 100 + 200, 0, 0, 0, 0};
  std::array<CPU::value_type, CPU::REGISTERS_COUNT> actual = cpu.getRegisters();
