...
#define TEST_(str)                                     \
  do {                                                 \
    std::cout << "Start testing " << str << std::endl; \
  } while (0);
#define END_()                                  \
  do {                                          \
...
int main(int argc, char* argv[]) {
  ...
  TEST_("unique_ptr") {
    ...
    // unique_ptr<Foo> IWhantToCreateThisPointer = p.at(0); // It does not
    // compile
    // unique_ptr<Foo> IWhantToCreateThisPointer2; IWhantToCreateThisPointer2 =
    // p.at(0); // It does not compile

    std::for_each(p.begin(), p.end(),
                  [](unique_ptr<Foo>& p) { std::cout << *p << "\n"; });
  }
  END_()
